...
I have also made a utility called [dat_file_parse](dat_file_parse.c) that will extract the names of each of the data items from a DAT file.
Note that some data items do not have names, in such cases the utility will mark their name as `NULL`.
...
This is a not a proper type, but rather a collection of offsets and a linked-list-like data structure.
This is used to make loading items by name more efficient.
...
### Item Hash Map Internals

The offset list for the hash map is always `0x400` bytes big.
256 entries, each on word large.
There are 256 entries because the hash funciton used returns a single byte.
Refer to the `name_hash` function found in [dat_file_parse](dat_file_parse.c) to see how it works.
Non-zero words in the offset list point to a an array of hashmap entry values.

```cpp
struct item_hashmap_entry {
    uint32_t item_type; // so 0, 2, 3, 6, or 7
    uint32_t item_index;
};
```

Each list in the hash map ends with an entry where both `item_type` and `item_index` `0xFFFFFFFF` (`-1`).
