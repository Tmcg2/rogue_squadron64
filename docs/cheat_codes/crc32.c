...
// I ashamedly cannot remember where I copied this table from :/
uint32_t crc_table[] = {
...
uint32_t func_800824F8(unsigned char* arg1, uint32_t arg2) {
    uint32_t var_a3;
    uint32_t var_t0;
    uint32_t var_v0;
    ...
    for (var_a3 = 0; var_a3 < arg2; var_a3++) {
        var_v0 = var_t0 ^ arg1[var_a3];
        var_t0 = (crc_table[var_v0 & 0xFF]) ^ (var_t0 >> 8);
    }
    return var_t0;
}
...
int main(int argc, char *argv[]) {
    for(int i; i < 30; i++) {
        printf("%#010x: %s\n", func_800824F8(codes[i], strlen(codes[i])), codes[i]);
    }
    return 0;
}
